...
import { InkButton, WaxSeal, ActionListRow } from './ShipyardUI';
...
export const ShipyardWindow: React.FC<ShipyardWindowProps> = ({ player, onClose, onAction }) => {
                ...
                {Object.entries(BOAT_UPGRADES).map(([key, def]) => {
                    ...
                    return (
                        <ActionListRow
                            ...
                            onAction={() => onAction({ type: 'UPGRADE_BOAT_COMPONENT', componentId: key })}
                        />
                    );
                })}
            </div>
    ...
    const ModelsView = () => {
        return (
            <div className="flex flex-col gap-6 p-6 overflow-y-auto max-h-[600px]">
                                    ...
                                    <div className="text-right flex flex-col items-end gap-3 pl-8 min-w-[200px]">
                                        ...
                                        <div className="flex flex-wrap justify-end gap-2 mt-2">
                                            {Object.entries(model.cost).map(([res, amt]) => (
                                                <div key={res} className="bg-stone-800/10 rounded px-2 py-1">
                                                    <ResourceIcon resource={res} amount={amt as number} size="sm" />
                                                </div>
                                            ))}
                                        </div>
                                    </div>
                    ...
                    <div className="flex gap-4 flex-wrap">
                        {COSMETIC_UNLOCKS.colors.map(c => {
                            ...
                            return (
                                <button
                                    key={c.id}
                                    onClick={() => onAction({ type: 'BUY_BOAT_COSMETIC', cosmeticType: 'color', id: c.id })}
    ...
    return (
        <SimulationMapWindow title="" onClose={onClose} maxWidth="max-w-7xl" hideHeader={true} noPadding={true}>
            <div className="flex flex-col h-[700px] w-full overflow-hidden rounded-b-xl shadow-2xl bg-[#0f0a05]">
                ...
                <div className="flex-1 grid grid-cols-[40%_60%] overflow-hidden bg-[#0c0a09]">
                    ...
                    <div className="relative bg-[#d6cbb8] flex flex-col shadow-[inset_10px_0_30px_rgba(0,0,0,0.3)]">
                        {/* Paper Texture */}
