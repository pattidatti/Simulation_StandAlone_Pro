...
export const SimulationMapWindow: React.FC<SimulationMapWindowProps> = ({
    title, subtitle, icon, headerRight, children, onClose, className, maxWidth,
    hideHeader = false,
    noPadding = false
}) => {
    useEffect(() => {
    ...
    return (
        <div className="absolute inset-0 z-40 flex items-center justify-center p-4 md:p-8">
            ...
            >
                ...
                <div className={`flex-1 overflow-y-auto custom-scrollbar ${noPadding ? '' : 'p-6'} ${className || ''}`}>
                    {children}
                </div>
            </motion.div>
        </div>
